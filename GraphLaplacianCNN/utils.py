...
class Calculate:
    @staticmethod
    def config_laplacian_3d(frames: np.ndarray[np.ndarray[np.ndarray[np.float32]]]) -> sp.sparse.csr_matrix[np.float32]:
        adj_matrix =\
    ...
    @staticmethod
    def adj_to_laplacian(adj_matrix: sp.sparse.csr_matrix[np.float32], method: str) -> sp.sparse.csr_matrix[np.float32]:
    ...
    def _spatial_splice(frames: np.ndarray[np.ndarray[np.ndarray[np.float32]]], k: int, sigma2: float) \
        ...
        for i, neighbors in enumerate(index_matrix):
            for j in neighbors:  # for each neighboring pair:
                ...
                adj.append([val, j, i])
        adj = np.array(adj)
        return sp.sparse.csr_matrix((adj[:, 0], (adj[:, 1], adj[:, 2])),
    ...
    def _k_nearest(shape: tuple[int, int], k: int) -> np.ndarray[np.ndarray]:
